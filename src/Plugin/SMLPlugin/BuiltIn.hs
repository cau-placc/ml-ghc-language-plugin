...
raise :: (ShowND a, Typeable a, Shareable SML a, Shareable SML b)
      => SML (a --> b)
raise = rtrnFunc $ \a -> show P.>>= \(Func f) -> f a P.>>= \f' -> nf f' P.>>= \s ->
  throwStrict a s
...
instance Normalform SML a1 a2 => Normalform SML (ListND a1) [a2] where
  ...
  embed xs' = case xs' of
    []   -> Nil
    x:xs -> Cons (P.return (embed @SML x)) (P.return (embed @SML xs))
...
instance (Normalform SML a1 a2, Normalform SML b1 b2) =>
  Normalform SML (Tuple2ND a1 b1) (a2, b2) where
    ...
    embed (a, b) = Tuple2 (P.return (embed @SML a)) (P.return (embed @SML b))
...
instance (Normalform SML a1 a2) =>
  Normalform SML (RatioND a1) (P.Ratio a2) where
    ...
    embed (a P.:% b) = P.return (embed @SML a) :% P.return (embed @SML b)
...
-- Pattern match failure is translated to an IO failure
pE :: ShareableN a => SML (ListND Char --> a)
pE = rtrnFunc (\l -> l P.>>= \l' -> nf l' P.>>= \s -> SML $ lift $ lift $ P.fail s)
